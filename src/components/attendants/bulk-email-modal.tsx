...
const BulkEmailModal: React.FC<BulkEmailModalProps> = ({
  ...
  const handleSendEmails = async () => {
    ...
    const emailsRemaining = 300 - emailsSentToday

    if (emailsRemaining <= 0) {
      setStatusMessage({
        type: "error",
        message: "Tu configuración actual solo permite 300 correos al día. Límite alcanzado.",
      })
      return
    }
    ...
    // Limitar la cantidad de correos a enviar según lo disponible
    const emailsToSend = Math.min(attendeesWithEmail.length, emailsRemaining)

    if (emailsToSend < attendeesWithEmail.length) {
      setStatusMessage({
        type: "info",
        message: `Solo se enviarán ${emailsToSend} correos de los ${attendeesWithEmail.length} seleccionados debido al límite diario.`,
      })
    }
    ...
    setStatusMessage({
      type: "info",
      message: `Enviando correos a ${emailsToSend} asistentes...`,
    })
    ...
    for (let i = 0; i < emailsToSend; i++) {
      ...
      if(emailsSentToday + successCount >= 300) {
        setStatusMessage({
            type: "error",
            message: `Se alcanzó el límite diario de 300 correos. Proceso detenido.`,
          })
          break 
      }
      ...
      try {
        ...
        if (updateError) {
          console.error(`Error al actualizar estado de notificación para ${attendee.email}:`, updateError)
        }
      ...
      // Update progress
      const newProgress = Math.round(((i + 1) / emailsToSend) * 100)
    ...
    if (emailsToSend === attendeesWithEmail.length) {
      ...
      setStatusMessage({
        ...
        message: `Proceso completado: ${successCount} correos enviados correctamente${
      ...
      setStatusMessage({
        type: "info",
        message: `Se enviaron ${successCount} correos. Has alcanzado el límite diario de 300 correos.${
          failedCount > 0 ? ` ${failedCount} correos fallaron.` : ""
        }`,
      })
    }
    ...
    // Notificar al componente padre que se han enviado correos
    if (onEmailSent && successCount > 0) {
      onEmailSent()
    }
  }
            ...
            <>
              <Alert className="bg-blue-50">
                ...
                <AlertDescription className="text-blue-800">
                  {emailsSentToday}/300 correos enviados hoy. Disponibles: {Math.max(0, 300 - emailsSentToday)}
                </AlertDescription>
              </Alert>
              ...
              {emailsSentToday >= 300 && (
                <Alert className="bg-red-50">
                  ...
                  <AlertDescription className="text-red-800">
                    Has alcanzado el límite de 300 correos diarios.
                  </AlertDescription>
                </Alert>
              )}
              ...
              <div className="border rounded-md overflow-hidden">
                <div className="bg-gray-50 p-2 border-b flex items-center">
                  <Checkbox
                    ...
                    disabled={emailsSentToday >= 300}
                  />
                  <Label htmlFor="select-all" className="ml-2 flex-1 cursor-pointer">
                    Seleccionar todos ({filteredAttendees.length})
                  </Label>
                  <div className="text-sm text-gray-500">{selectedAttendees.length} seleccionados</div>
                </div>
                      ...
                      {filteredAttendees.map((attendee) => (
                        <div key={attendee.id} className="p-2 flex items-center hover:bg-gray-50">
                          <Checkbox
                            ...
                            disabled={!attendee.email || attendee.notificated || emailsSentToday >= 300}
                          />
                          <Label
                          ...
                          <code className="text-xs bg-gray-100 px-2 py-1 rounded">{attendee.code}</code>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-center py-8 text-gray-500">No se encontraron asistentes</div>
                  )}
                </div>
              </div>
          ...
          {isSending ? (
            ...
            <>
              ...
              <Button
                ...
                disabled={selectedAttendees.length === 0 || emailsSentToday >= 300}
